...
| Panning for Gold Skill | ✅ Complete | `.claude/commands/pan.md` — `/pan <url or text> [--dry-run]`. 3-phase Extract → Evaluate → Synthesize. Fetches YouTube transcripts via transcript tool, captures keepers to brain. |
| Meeting Synthesis Skill | ✅ Complete | `.claude/commands/synth.md` — `/synth [--dry-run] [--meeting "title"]`. Extracts decisions, action items, open questions, context, and risks from raw meeting notes and captures each to brain. |
| Auto-capture Skill | ✅ Complete | `.claude/commands/recap.md` — `/recap`. Run at end of work session. Summarizes commits + file changes + session context, captures built/decisions/learnings/open threads to brain. |
