...
- **`project_scoping_field.md`** — **designed 2026-07-08 via grill-me** (was a stub needing a planning session; now specced). Adds a nullable `workspace` column to `thoughts` so retrieval defaults to the current project instead of returning cross-project noise. Derivation: `external-gate → .workspace override file → git-toplevel basename → cwd basename → null`; retrieval scoped-by-default (`current OR null`) with `workspace:"all"` override. 7 touch points (migration `007`, Edge Function, MCP capture + query tools, `semantic_search` RPC, `brain.py`, `.workspace` files in ABU repos). Backfill mapping verified against live DB (`agile_backlog_builder`=19, `abucw`=18, `idea_center_ai_policy`=6, 13 external Substack rows excluded/null). Also captured a **separate non-blocking bug**: `is_external` mislabeling (~213 external rows flagged false). Unblocks `recall_before_work_skill.md`.
