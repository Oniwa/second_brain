...
## Active
**Nothing in flight.** `compile_wiki.py`'s pagination bug was just fixed and verified (see Recently Shipped) — pick the next item from `plans/roadmap.md` (now #1: wiring up the weekly wiki cron). One deliberately-deferred follow-up from that fix: no full `--all` recompile has been run yet against the corrected data — it's a real-cost action (417 pages, all Sonnet calls) left for an explicit decision, likely bundled with the weekly-cron item rather than run ad hoc.
...
- **Decide cron location** — Pi vs PC vs on-demand; blocks scheduling weekly wiki recompile — see `plans/in_progress/wiki_weekly_cron.md`
...
- **`wiki_weekly_cron.md`** (new stub, 2026-07-20) — everything needed to automate `compile_wiki.py --all` is already built; only blocked on confirming the Pi is actually fine for this (the "locks to home network" framing may not hold given digest/nudge/remind already run there against the same cloud services). Now unblocked by the pagination fix — the first automated run will compile against correct counts.
