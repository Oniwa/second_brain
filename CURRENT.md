...
## Active
**Nothing in flight — weekly wiki cron plan finalized, ready to implement (roadmap #1).** The grill-me pass on `plans/in_progress/wiki_weekly_cron.md` (2026-07-22) resolved every open question (Pi location, Sunday 3am `0 3 * * 0`, `--all --skip-unchanged` no `--strict`, one-line Discord DM User-Agent fix folded in). Nothing built yet — blocked only on switching to the PC with RPi access. Order when picked up: (1) add User-Agent header to `compile_wiki.py` `send_discord_dm`; (2) run `compile_wiki.py --all` once, manually + watched, for the one-time 417-page correction against the fixed counts; (3) add the job to `setup_rpi.py`; (4) re-run `setup_rpi.py` on the Pi. The deferred full `--all` recompile is step 2 of that sequence.
