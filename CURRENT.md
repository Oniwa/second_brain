...
- **`get_pans_skill.md`** (new stub) — Haiku-driven `get-pans` skill to auto-populate `open_pans.md` from the second brain, replacing the current manual reconciliation. Builds on `mcp_improvements.md` §5's prior analysis (the reliable query primitive already exists). Needs a grill-me pass — biggest open question is overwrite-vs-merge with the existing file's valuable closed-out history
