...
**Caught mid-cleanup: the edge function fix was committed to git but not actually deployed.** `process-thought` and `generate-digest` were still running the pre-fix code in production (still on the versions deployed right after the model swap, before the parsing bug was found) — meaning any live Sonnet escalation or scheduled digest would have hit the same `KeyError: 'text'`. Redeployed and confirmed live: `process-thought` → v14, `generate-digest` → v6.

**Blockers cleared 2026-07-24 — ready for the full `--all` correction.** (1) The adaptive-thinking question is settled: A/B tested on "AI agents" and **closed as keep-disabled** — adaptive exhausts `max_tokens: 8192` on the largest pages and returns zero text (crashed; the one failed call cost $0.81 vs $0.52 for all three disabled smoke pages). The temporary `--thinking` flag was **removed** so it can't be turned on accidentally (`plans/done/wiki_thinking_ab_test.md`). (2) `compiled_wiki` (underscore) cloned on Linux and confirmed as a sibling of `scripts/`; the stale `compiled-wiki` (hyphen, May-9 leftover) was deleted and `.gitignore` corrected to the underscore name. The **Step 2 smoke test passed** (3 single-entity real calls — AI agents / Tammy / Second Brain — all clean, correct folders, DB rows updated, $0.52). Remaining: run `compile_wiki.py --all` (the full ~418-page correction, watched, ~$15–25), then finish the cron plan's Step 5 (re-run `setup_rpi.py` on the Pi to pick up the cron job already added to it).
