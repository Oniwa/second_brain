...
- OB1 deduplicates at ingestion; second_brain does not
...
**second_brain:** Local stdio process, 10 tools — `semantic_search`, `list_recent`, `capture_thought`, `get_stats`, `update_thought`, `archive_thought`, `delete_thought`, `get_thought`, `get_context`, `meeting_prep`.
...
- second_brain has far richer tooling (update, archive, delete, meeting prep, get_context, full raw text retrieval)
...
- Content fingerprint deduplication at ingestion
...
1. **Content fingerprint deduplication** — hash raw text before insert to prevent duplicate thoughts
2. **Configurable similarity threshold** — expose as parameter on `semantic_search` instead of hardcoded value
3. **OpenRouter as a gateway** — single API key, model-agnostic, easier to swap models

### Medium Value / More Work
4. **Remote HTTP MCP server** — Hono + StreamableHTTP transport; pairs well with planned Python rewrite
5. **Data import recipes** — migrate historical data from ChatGPT history, Gmail, Obsidian, Google Takeout
6. **Better dashboard** — Next.js or SvelteKit with thought editing, duplicate management, audit view

### Lower Priority / Nice to Have
7. **Row Level Security + shared MCP** — scoped access for other users
8. **pg_cron scheduling** — remove Pi as single point of failure for digests and nudges
