...
**Leading candidate, same session: `plans/in_progress/wiki_cowork_synthesis.md`.** Route expensive pages through Claude Cowork's Scheduled Tasks instead of trading freshness for cost — confirmed (not assumed) subscription billing, genuine cloud-hosted scheduling, Supabase connector, full Windows parity. The only option considered that hits cheap + automatic + fresh simultaneously with actual product confirmation. Needs a proof-of-concept (one real scheduled task against `AI agents`) before design work starts; the plain delta-threshold remains the fallback if it doesn't pan out.
