...
**New follow-up opened 2026-08-07, during the cron's first live activation run.** The any-delta-triggers-full-recompile problem the cadence fix solved for content-creator people turns out to be general — topics (and unverified, projects) still use the plain any-delta rule. First live run cost ~$4 and had to be resumed after an interrupt: ~40 topics had genuine but *tiny* deltas (+1 to +6 thoughts each, e.g. `AI skills` 13→14), yet each triggered a full re-synthesis of the page's entire thought volume — the big topics (`AI agents` 331, `prompt engineering` 138, `system design` 112, ...) accounted for essentially all of the cost. This is Option 2 from the original plan ("change threshold instead of any-delta"), previously resolved only for people via cadence; needs its own grill-me pass to decide whether/how a delta-magnitude threshold applies to topics and projects too.
