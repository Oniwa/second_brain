...
**Accepted risk — concurrent writes during a multi-page fetch.** Offset-based pagination is not a stable snapshot: each page is its own independent HTTP request/transaction, so a thought captured or archived (via Discord/MCP) *while* a multi-page loop is in flight can cause the offset to drift — a row could be silently skipped or double-counted between page 1 and page 2 of the same run. `id.asc` ordering fixes determinism *within* a snapshot, not changes *between* requests. Accepted rather than fixed for this pass: this is a low-write-volume personal system, the window between page fetches is seconds, and the total-count verification against `execute_sql` ground truth (Final Design #7) would likely surface it if it were happening in practice. If it ever does become a real problem (e.g. once weekly cron + concurrent Discord capture activity overlap more), the fix is a snapshot cutoff — filter every page to `created_at <= run_start_timestamp` — deferred until there's evidence it's needed.
