...
> ⛔ **GATE, re-opened 2026-08-07 — do NOT activate the cron yet.** The original roadmap #7 gate (Nate B. Jones cost) was resolved and shipped 2026-08-06, and Steps 1–4 plus everything through smoke-testing `--all --skip-unchanged --git-publish` live on the Pi were verified working correctly that session (cadence gate, git-publish, Discord DM reporting all confirmed against real production data — see `CURRENT.md` 2026-08-07). **But the live smoke-test run itself cost ~$7** for what was mostly a light two-week catch-up window (few pans, mostly incidental topic drift) — a genuine signal, not noise, about steady-state weekly cost under the current metered-API architecture. **Modeled steady-state estimate** (calibrated against actual 8-week per-page capture rates and today's real bill, content-creator pages correctly excluded since they're on cadence): **~$4.60/week, ~$20/month** — rough (assumes recent capture patterns hold, cost model unverified against actual billing), but the right order of magnitude to plan around. That run also surfaced that the topics/projects side has the *same* any-delta-triggers-full-recompile problem the people-side cadence fix solved, but **topics have no duplication-based escape hatch** the way Nate did (measured: high topic-to-topic overlap is normal multi-tagging, not redundancy) — so a cadence/threshold fix would mean accepting staleness on exactly the highest-value pages. The leading fix under design is **`plans/in_progress/wiki_cowork_synthesis.md`** — routing expensive pages through Claude Cowork's Scheduled Tasks (subscription billing, not metered API) instead of trading freshness for cost. **Activating this cron now would commit to weekly metered spend on the exact mechanism likely to be partially replaced.** Hold Step 5 until the Cowork routing decision is made (or explicitly decided against).
>
> Once unblocked: re-run `setup_rpi.py` on the Pi (or manually patch `/etc/cron.d/second-brain`) to pick up the job. Also worth folding in before/at that point: a real case-collision bug found 2026-08-07 — `slugify()` lowercases topic names, so e.g. `"Docker"` and `"docker"` produce the identical slug and silently overwrite each other's `wiki_pages` row every run, causing a small but permanent recompile-oscillation. Fix is a one-line `topic_aliases.json` addition (`"docker": "Docker"`), exactly mirroring the existing `"MTG"` entry in that file — cheap, unrelated to the Cowork decision, safe to do independently.
