...
**Superseded by a stronger candidate, same session.** A delta-threshold fix was going to force the same bad trade the cadence fix avoided for people — accept staleness on exactly the pages that are large *because* they're high-value (measured: the biggest topics have no duplication-based escape hatch the way Nate did; see the design doc for the overlap check). The leading direction is now **`plans/in_progress/wiki_cowork_synthesis.md`** — routing expensive pages through Claude Cowork's Scheduled Tasks (confirmed: subscription billing not metered API, genuine cloud-hosted scheduling, Supabase connector, full Windows parity) instead of accepting a cost/freshness trade at all. Needs its own proof-of-concept before design; this file's delta-threshold options remain the fallback if Cowork doesn't pan out.
