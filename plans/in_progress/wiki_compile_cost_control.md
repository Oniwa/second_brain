...
## Follow-up opened 2026-08-07 — any-delta-triggers-full-recompile is general, not just Nate

The first real Pi run (`--all --skip-unchanged --git-publish`, cron activation) cost ~$4 and had to be interrupted (Ctrl-C) partway through. Root cause: this was the cron's first-ever live run — the last full topic recompile was 2026-07-24, so ~2 weeks of ordinary capture had drifted many topics' `thought_count` by small amounts. Checked directly against production (`--dry-run`): **~40 topics with real deltas, all +1 to +6 thoughts** (e.g. `AI skills` 13→14, `ABU consolidation` 6→12) — not large content growth. But `--skip-unchanged`'s any-delta check forces a **full recompile of the entire page's current thought volume** regardless of delta size, so `AI agents` going 330→331 re-synthesizes all 331 thoughts, not the 1 that changed. The big topics (`AI agents` 331, `prompt engineering` 138, `system design` 112, `Claude Code` 106, `automation` 99, `knowledge management` 99, ...) are where essentially all of the $4 went.

This is **option 2 from the original Options list above** ("change threshold instead of any-delta"), rejected in the 2026-08-06 resolution *for content-creator people specifically* (superseded by cadence) — but never evaluated for topics/projects, which still run the plain any-delta rule noted in `wiki_weekly_cron.md`'s "Known limitations (accepted)". That acceptance was written before this was observed costing real money; worth re-opening now that it has a concrete price tag.

**Not solved in the run that surfaced it** — deliberately deferred rather than designed under time pressure mid-cron-activation. Resume-and-finish was chosen over stop-and-redesign since the expensive part (big topics) was already sunk cost and the remainder was cheap.

**Needs its own grill-me pass before implementing.** Open questions: absolute delta vs. percentage vs. hybrid; does this apply to projects too (project pages showed 0 deltas this run, unverified at scale); does it compound with or replace the existing per-page `thought_count`-equality check; should it use the same `--cadence-days`-style CLI override pattern already established for people.

**Superseded by a stronger candidate, same session.** A delta-threshold fix was going to force the same bad trade the cadence fix avoided for people — accept staleness on exactly the pages that are large *because* they're high-value (measured: the biggest topics have no duplication-based escape hatch the way Nate did; see the design doc for the overlap check). Two alternative directions were evaluated instead of accepting that trade:

1. **Leading candidate: `plans/in_progress/wiki_incremental_compile.md`** (2026-08-07) — synthesize from only the thoughts captured since the last compile plus the existing compiled page, instead of the full raw corpus every time. Quantified against real data: input tokens for "AI agents" would drop from ~128K (full corpus) to ~9-10K (existing page + delta), an ~85-90% reduction; modeled monthly cost drops from ~$20 to an estimated ~$3-5. No new platform or auth needed. Real risk: quality drift over repeated incremental edits without full reconsideration — mitigated by a periodic full-corpus re-baseline, mirroring the cadence pattern already shipped for people. **Try this first.**
2. **Fallback: `plans/in_progress/wiki_cowork_synthesis.md`** — routing expensive pages through Claude Cowork's Scheduled Tasks (subscription billing, not metered API). Sidesteps the drift question entirely (keeps full-resynthesis quality) but needs a new platform, auth, and has unverified context/usage-pool fit. Revisit only if incremental compile's drift risk proves unmanageable.

This file's plain delta-threshold options remain the last-resort fallback if neither works out.
